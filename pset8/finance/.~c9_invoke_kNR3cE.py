...
def index():
    ...
    for stock in stocks:
        ...
        stock.update({"total": usd(stock["total"])})
    grandtotal += user["cash"]

    return render_template("index.html", rows=stocks, grandtotal = usd(grandtotal), user=user, cash = usd(user["cash"]))
...
def buy():
    ...
    db.execute("""
        INSERT INTO transactions (id, symbol, company_name, shares, price, total, date)
        VALUES(?,?,?,?,?,?,?)
        """, userid, symbol, quote["name"], shares, quote["price"], total, date)
...
def quote():
    if request.method == "GET":
        ...
        #return apology(quote["name"], 403)
        return render_template("inquiry.html", quote=quote)
